...
function openUtility(key){
  document.querySelectorAll('.utility-nav button').forEach(b=>b.classList.toggle('active',b.dataset.utility===key));
  document.querySelectorAll('.utility-pane').forEach(p=>p.classList.toggle('active',p.dataset.pane===key));
}
document.querySelectorAll('.utility-nav button').forEach(b=>b.addEventListener('click',()=>openUtility(b.dataset.utility)));

document.querySelectorAll('[data-go]').forEach(el=>{
  el.addEventListener('click',()=>{
    showView(el.dataset.go);
    if(el.dataset.openUtility) setTimeout(()=>openUtility(el.dataset.openUtility),120);
  });
});
...
document.querySelectorAll('.source-item').forEach(item=>{
  item.addEventListener('click',()=>{
    ...
    const pane=document.querySelector(`.utility-pane[data-pane="${cat}"]`);
    if(!pane) return;
    ...
    if(img){img.style.opacity='.25';img.src=item.dataset.thumb;img.onload=()=>img.style.opacity='1';}
